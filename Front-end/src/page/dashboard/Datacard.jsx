...
import { useGetEnvironmentConditionQuery } from "../../lib/redux/Query";
...
const Datacard = () => {
   ...
   const weatherData = [
    ...
    const {data:environmentCondition, error:envError, isLoading:envLoading ,isError:envIsError} = useGetEnvironmentConditionQuery({
        API_KEY:"a8590efdefee4d0a948145110252710",
    });
  ...
  return (
    <div className="datacard">
      ...
      <div className="w-full max-w-screen-lg mx-auto p-4 relative mt-4">
            <img src={Enviromentcondition} alt="Environment Condition"  className="w-80 h-64 sm:w-80 sm:h-48 md:w-96 md:h-56 lg:w-[500px] lg:h-[320px]  
                object-cover
                rounded-lg
                shadow-md
                brightness-50
            ...
            <div className="absolute bottom-4 ml-9 mb-2">
                ...
                <div>
                    <p className="text-xl md:text-2xl font-bold text-white">
                       {environmentCondition?.current?.[item.parameter]}
                       {item.unit}
                    </p>
        ...
        <h1 className="text-xl font-semibold text-black">Last 7 Days Energy Consumption</h1>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-7 ">
                    {energyGenerationRecords.map((record) => (
                        <div key={record.date} className="bg-gray-200 p-4 rounded-md shadow flex items-center justify-center mt-4 hover:bg-gray-400 transition-colors">
                            <div>
